...
#plot "1_4_D.dat" u (bin($1,binwidth)):(1.0) notitle smooth freq with boxes #histogramm plotten
...
#fit f(x) "1_4_D.dat" using 1:2:4 via m,b #funktionen fitten
plot "1_4_D.dat" with xyerrorbars title "Messdaten"#, f(x) title "Regressionsgerade"#, g(x) title "Theoriekurve" #, "2_12.dat" smooth csplines title "Messdaten mit csplines" #plotten von messdaten, regression, und csplines

set terminal postscript eps enhanced color font 'Helvetica,14' #ausgabeformat
set output "1_4_D.eps" #name des dokuments
replot #dokument plotten
!epstopdf 1_4_D.eps #eps in pdf umwandeln
