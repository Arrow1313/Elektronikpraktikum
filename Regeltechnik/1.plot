...
#plot "1.dat" u (bin($1,binwidth)):(1.0) notitle smooth freq with boxes #histogramm plotten
...
g(x) = 10000*exp(-3988*((1/293.15)-(1/x))) 

a = 4239.75

f(x) = 10000*exp(-a*((1/293.15)-(1/(x)))) #funktionen definieren
fit f(x) "1.dat" using 1:2:4 via a #funktionen fitten
plot "1.dat" with xyerrorbars title "Messdaten", f(x) title "Regression" , g(x) title "Theoriekurve" #, "1.dat" smooth csplines title "Messdaten mit csplines" #plotten von messdaten, regression, und csplines

set terminal postscript eps enhanced color font 'Helvetica,14' #ausgabeformat
set output "1.eps" #name des dokuments
replot #dokument plotten
!epstopdf 1.eps #eps in pdf umwandeln
